# These functions used to be in src/make_PCA.py
...
def eigenworm_PCA_analysis(K_df, PCA_model, segments: list = None, output_path: str = None):
    ...
    if int(PCA_model.n_features_) is not (last_body_part - first_body_part):
        ...
        print("recieved: " + str(last_body_part - first_body_part) + " features but the model has " + str(
            PCA_model.n_features_) + " features!")
...
def plot_eigenworm_PCA_analysis(eigenworm_PCA_df: pd.DataFrame, num_PC_to_plot: int = 2, start_frame: int = 0,
                                end_frame: int = None, ylim: list = None, xlim: list = None, output_path: str = None):
    """
...
def anotate_reversals_wCrossProduct(eigenworm_PCA_df: pd.DataFrame, rol_mean_window: int = 5, output_path: str = None,
                                    plot_crossproduct: bool = False):
    """
    ...
    for i, row in enumerate(cross_product_df.iterrows()):
        ...
        r = np.cross([cross_product_df['X'].values[i], cross_product_df['Y'].values[i]],
                     [cross_product_df['X'].values[i + 1], cross_product_df['Y'].values[i + 1]])
        ra.append(r)
    cross_product_df['Cross Product'] = ra

    cross_product_df_rolMean = cross_product_df.loc[frames, 'Cross Product'].rolling(window=rol_mean_window,
                                                                                     center=True).mean()
...
def annotate_reversals_wCrossProduct2(eigenworm_PCA_df: pd.DataFrame, rol_mean_window: int = 5,
                                      interpolate_window: int = 5, return_cross_product: bool = False,
                                      debug: bool = False):
    """
    Anotate reversals using the cross product between PCs of two consecutive frames.
    the function recieves a dataframe holding the eigenworm principal components per frame
    ...
    # if there's something to interpolate then interpolate then rolling mean
        ...
        cross_product_df_interpolated = cross_product_df.loc[frames, 'cross_product'].interpolate(
            limit=interpolate_window)
