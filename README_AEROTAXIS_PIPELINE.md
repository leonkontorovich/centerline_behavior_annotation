...
4. **Missing `track.txt` absolute positions**
...
> **Run it on the cluster, not over a mounted share.** It reads one DLC `.h5`
> per crop, so it is I/O-bound: on the LiSC login node a full dataset takes a
> couple of minutes, while the same scan over a mounted `/Volumes/scratch`
> took >20 min and had to be abandoned. If you must run it over a mount, scope
> it to a few recordings (`calibrate_min_worm_length.py <recording>_new`) —
> five recordings are plenty to set a junk floor.
...
6. **`finalize_aerotaxis_dataset.sh` cannot pass `--genotype-regex`.** Its
   `-- <extra args>` are forwarded to `aerotaxis_analysis.py` only
   (`finalize_aerotaxis_dataset.sh:55`); `create_results_dict_server.py` is
   always called with just `--format`. So a dataset whose folder names need a
   custom regex must run the combiner by hand first, then the analysis. A
   forwarding flag would be a few lines.

7. **Four recording folders still contain a literal space on disk**
   (`..._N2_ B`, `..._rde_ B`, `..._mut_ B`, `..._npr_ B`). Genotype/plate
   parsing now normalises this, so they group correctly — but the space still
   travels into the `Recording` column and into every quoted path. Renaming the
   inner folders before the re-run is cheap and removes a whole class of
   quoting bugs.

8. **Fragment tracking: `Crop_ID` is not an animal.** SWC assigns a brand-new
...
9. **No end-to-end test of the Snakemake DAG.** The 56 unit tests cover the
...
10. **`quick_status.sh` cannot distinguish "failed" from "not started".** It
...
11. **`downsample_factor` in `config.yaml` is vestigial here** — it is read by
...
12. **`t0_offset_s` is still hand-entered.** It is the single most damaging value
