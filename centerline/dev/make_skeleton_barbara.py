...
#create csv objects
output_path='/groups/zimmer/shared_projects/Barbara_Ulises/Labeller_Test/multiple_skeleton/output_skeletons2/'+args['input_filename'][-14:-5]#'/groups/zimmer/Ulises/code/skeleton_outputs/shortest_path2'

csvfilePathX=open(output_path+'skeleton_X_coords.csv','w', newline='')
csv_writerPathX=csv.writer(csvfilePathX)

csvfilePathY=open(output_path+'skeleton_Y_coords.csv','w', newline='')
...
csvfileK=open(output_path+'spline_K.csv','w', newline='')
...
#             img[x][y]=0
        ...
        if False:#len(pts)<knots:
            ...
            tck, u = splprep(pts.T, u=None, s=pts.shape[0]/2, per=0, k=5)#k=5 for my data
            #print(len(tck[1]))
