...
snakemake --configfile config.yaml --latency-wait 3600 --cluster "$OPT" --cluster-config cluster_config.yaml --jobs $NUM_JOBS_TO_SUBMIT --rerun-incomplete
#snakemake --configfile config.yaml --latency-wait 120 --use-conda --cores 1 
#snakemake --configfile config.yaml --latency-wait 120 --use-conda --cores 1 --unlock --touch
#snakemake --configfile config.yaml --latency-wait 120 --cores 1 -p 