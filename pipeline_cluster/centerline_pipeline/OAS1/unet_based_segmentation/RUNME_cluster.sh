...
snakemake --configfile config.yaml --latency-wait 500 --cluster "$OPT" --cluster-config cluster_config.yaml --jobs $NUM_JOBS_TO_SUBMIT --rerun-incomplete --forcerun create_plots2
...
#snakemake --configfile config.yaml --latency-wait 120 --cores 1 --delete-all-output --rerun-incomplete