...
try:
    ...
    with open(raw_data_config_fname, 'r') as f:
        raw_data_config = YAML().load(f)
except FileNotFoundError:
    raw_data_config_fname = "NOTFOUND_raw_data_config_fname"
    raw_data_config = dict(ventral='NOTFOUND')
...
rule traces_and_behavior:
    input:
        ...
        chemotaxis_done=f"{output_behavior_dir}/chemotaxis_analysis.done"
...
rule invert_curvature_sign:
    ...
    params:
        ventral = raw_data_config['ventral'],
    output:
...
rule annotate_turns:
    input:
        spline_K = f"{output_behavior_dir}/skeleton_spline_K__equi_dist_segment_2D_smoothed_signed.csv",
        beh_annotation = f"{output_behavior_dir}/beh_annotation.csv"
    params:
        ...
        min_event_length = config.get("turn_min_event_length", 24)  # Default to 24 if not in config
    output:
...
rule chemotaxis_analysis:
    ...
    params:
        factor_px_to_mm = raw_data_config["factor_px_to_mm"],
        video_resolution_x = raw_data_config["video_resolution_x"],
        video_resolution_y = raw_data_config["video_resolution_y"],
        fps = config["fps"],
        conc_gradient_array = raw_data_config["conc_gradient_array"],
        distance_array = raw_data_config["distance_array"],
        top_left_pos = raw_data_config["top_left_pos"],
        odor_pos = raw_data_config["odor_pos"],
        diffusion_time_offset = raw_data_config["diffusion_time_offset"],
        video_source = raw_data_config["video_source"],
        dlc_nose_label = config["nose"],
        dlc_tail_label = config["tail"],
        bootstrap_iterations = 100,
        bootstrap_seed = 42,
        dC_lookback_frames = 1,
        stage_pos = lambda wildcards: next((os.path.join(raw_data_dir, f) 
                                         for f in os.listdir(raw_data_dir) 
                                         if f.endswith("-TablePosRecord.txt")), "")
    ...
    output:    
        ...
        chemotaxis_h5 = f"{output_behavior_dir}/chemotaxis_analysis_complete.h5",  # New HDF5 output
        done = touch(f"{output_behavior_dir}/chemotaxis_analysis.done")
    shell:
        """
        unset DCGM_ARGS
        ...
        # Execute the Python script with arguments passed as a single quoted string
        python -c "from chemotaxis_analysis_high_res import initialize_load_files_high_res_universal_bootstrapping; initialize_load_files_high_res_universal_bootstrapping.main([\\
            '--reversal_annotation', '{input.reversal_annotation}', \\
            '--skeleton_spline', '{input.spline_K}', \\
            '--worm_pos', '{params.stage_pos}', \\
