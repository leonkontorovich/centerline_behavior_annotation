...
#snakemake --configfile config.yaml --latency-wait 500 --cluster "$OPT" --cluster-config cluster_config.yaml --jobs $NUM_JOBS_TO_SUBMIT --rerun-incomplete
snakemake --configfile config.yaml --latency-wait 120 --use-conda --cores 1 --rerun-incomplete
#snakemake --configfile config.yaml --latency-wait 120 --use-conda --cores 1 --forceall
