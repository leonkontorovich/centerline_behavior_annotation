...
class ImagePointDetector:
    ...
    def save_to_yaml(self, image_path, coords):
        base_dataset_folder = self.folder_path
        config_path = os.path.join(base_dataset_folder, "dataset_coordinates.yaml")
        ...
        try:
            ...
            if subfolder_key not in config:
                config[subfolder_key] = {}

            if 'top_left' in coords:
                x, y = coords['top_left']
                config[subfolder_key]['top_left_x'] = int(x)
                config[subfolder_key]['top_left_y'] = int(y)
                print(f"Saving top_left coordinate: ({x}, {y})")

            if 'odor_pos' in coords:
                x, y = coords['odor_pos']
                config[subfolder_key]['odor_x'] = int(x)
                config[subfolder_key]['odor_y'] = int(y)
            ...
            print(f"Coordinates saved successfully in {config_path}")
        except Exception as e:
    ...
    def next_image(self):
        image_path = self.image_paths[self.current_image_index]
        if image_path in self.points:
            self.save_to_yaml(image_path, self.points[image_path])
