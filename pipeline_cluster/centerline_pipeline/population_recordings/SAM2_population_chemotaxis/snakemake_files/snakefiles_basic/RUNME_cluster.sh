...
#snakemake --configfile config.yaml --latency-wait 500 --cluster "$OPT" --cluster-config cluster_config.yaml --jobs $NUM_JOBS_TO_SUBMIT --rerun-incomplete --keep-going --forcerun basic_analysis
...
snakemake --configfile config.yaml --latency-wait 500 --use-conda --cores 1 --forcerun basic_analysis